...
import VoucherOrder from "../models/voucherorders.mode.js"

dotenv.config();
const stripe = Stripe(process.env.CHECKOUT_API_KEY_SECRET);

let pid;// payid to send frontend
//create session for array of items
  ...
  const line_items = req.body.cartItems.map((item) => {
    return {
      price_data: {
        ...
        unit_amount: item.discountValue * 100,
      },
...
const createOrder = async (customer, data) => {
  pid = data.payment_intent; //pid to send frontend
